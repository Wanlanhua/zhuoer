<%@page import="com.zhuoer.newCareDevice.bean.Pdsdy"%>
...
<%
	...
	if(session.getAttribute("thispdsdyPage")!=null)
		thisPage = Integer.parseInt(session.getAttribute("thispdsdyPage").toString());
	if(pageSize%15==0)
...
<body>
<table width="100%" border="0" cellspPDSDYng="0" cellpadding="0" id="searchmain" >
  <tr>
    <td width="99%" align="left" valign="top">您的位置：配电室低压</td>
  </tr>
  <tr>
  <%
  	String classNone = "";
  %>
    ...
    <table width="100%" border="0" cellspPDSDYng="0" cellpadding="0" id="search">
  		<tr>
   		 <td width="90%" align="left" valign="middle">
	         <form id="form" method="post" action="<%=request.getContextPath() %>/newCareDevice/GetPDSDY?page=1">
	         <span>设备号：</span>
         ...
         %>
  		  <td width="10%" align="center" valign="middle" style="text-align:right; width:200px;display:<%=classNone%>">
  		  	<a href="Pdsdy.html?no=<%=request.getSession().getAttribute("name").toString() %>" target="mainFrame" onFocus="this.blur()" class="add" id="addDom">配电室低压</a>
  		  </td>
  		</tr>
	</table>
    </td>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspPDSDYng="0" cellpadding="0" id="main-tab">
      <tr  class="bggray">
      ...
      <%
      		List<Pdsdy> list = (List)request.getSession().getAttribute("pdsdylist");
      		if(list != null) {
				        ...
				        <td align="center" valign="middle" class="borderbottom">
				         ...
				         %>
				        	...
				        	<span class="gray" style="display:<%=classNone%>">&nbsp;|&nbsp;</span>
					         ...
					         %>
				        	<a href="<%=request.getContextPath() %>/DeletePDSDY?id=<%=list.get(i).getId() %>" style="display:<%=classNone%>" onclick="return delDate(<%=list.get(i).getId() %>, this)" target="mainFrame" onFocus="this.blur()" class="DeleteeteData">删除</a>
				        	<!--
				            	作者：923501316@qq.com
				            	时间：2018-04-18
				            	描述：如果该记录被审核，则允许再次审核
				            ...
				            %>
				        	<span class="gray" style="display:<%=classNone %>">&nbsp;|&nbsp;</span>
				        	<a href="<%=request.getContextPath() %>/SHPDSDY?id=<%=list.get(i).getId() %>&status=1" target="mainFrame" onFocus="this.blur()" class="stateData" style="display:<%=classNone %>">通过</a>
				        	<span class="gray" style="display:<%=classNone %>">&nbsp;|&nbsp;</span>
				        	<a href="<%=request.getContextPath() %>/SHPDSDY?id=<%=list.get(i).getId() %>&status=2" target="mainFrame" onFocus="this.blur()" class="stateData" style="display:<%=classNone %>">不通过</a>
				        </td>
		        <%
			        	}
		        	}
    ...
    %>
		  <tr>
		    <td align="left" valign="top" class="fenye">
	    		...
	    		<a href="<%=request.getContextPath() %>/newCareDevice/GetPDSDY?page=1" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
		    	<%
		    		int beforePage = thisPage - 1;
		    		if(thisPage>1) {
		    		
		    	%>
		    		<a href="<%=request.getContextPath() %>/newCareDevice/GetPDSDY?page=<%=beforePage %>" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;
		    	<% 
		    		}
		    	%>
		    	<%
		    	...
		    	%>
		    		<a href="<%=request.getContextPath() %>/newCareDevice/GetPDSDY?page=<%=afterPage %>" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;
		    	<%
		    		}
		    	%>
		    	...
		    	%>
		    		<a href="<%=request.getContextPath() %>/newCareDevice/GetPDSDY?page=<%=pageSize %>" target="mainFrame" onFocus="this.blur()">尾页</a></td>
		    	<%
		    		}
		    	%>
		  </tr>
	...
	<script>
	    ...
	    $( "#starttime" ).datepicker({//添加日期选择功能  
          ...
          $("#export").on("click",function() {
        	  ...
        	  console.log(params)
    		$.ajax({
				url: 'newCareDevice/GetPDSDYPage',
				type: 'get',
				data:params,
