window.onload = function() {
	...
	function transferenceData(datadoms) {
		for(var i=0,j=0; i<datadoms.length-1; i++,j++) {
			if(i==2) {
				var select = dataBoxDom.getElementsByTagName("select")[0];
				select.value = datadoms[i].innerHTML;
				j++;
			}
			dataInputsDom[i].value=datadoms[j].innerHTML;
		}
	}
	...
	$("body").on("click",".ui-menu-item",function() {
    	...
    	var content = content.substring(0,end);
    	$("#findByNo").val(content)
    })
}
