<%@page import="com.zhuoer.newCareDevice.bean.Kqysjyxjlb"%>
...
<%
	...
	if(session.getAttribute("thiskqysjyxjlbPage")!=null)
		thisPage = Integer.parseInt(session.getAttribute("thiskqysjyxjlbPage").toString());
	if(pageSize%15==0)
...
<body>
<table width="100%" border="0" cellspKQYSJYXJLBng="0" cellpadding="0" id="searchmain" >
  <tr>
    <td width="99%" align="left" valign="top">您的位置：空气压缩机</td>
  </tr>
  <tr>
  <%
  	String classNone = "";
  %>
    ...
    <table width="100%" border="0" cellspKQYSJYXJLBng="0" cellpadding="0" id="search">
  		<tr>
   		 <td width="90%" align="left" valign="middle">
	         <form id="form" method="post" action="<%=request.getContextPath() %>/newCareDevice/GetKQYSJYXJLB?page=1">
	         <span>设备号：</span>
         ...
         %>
  		  <td width="10%" align="center" valign="middle" style="text-align:right; width:200px;display:<%=classNone%>">
  		  	<a href="Kqysjyxjlb.html?no=<%=request.getSession().getAttribute("name").toString() %>" target="mainFrame" onFocus="this.blur()" class="add" id="addDom">空气压缩机</a>
  		  </td>
  		</tr>
	</table>
    </td>
  ...
  <tr>
    ...
    <table width="100%" border="0" cellspKQYSJYXJLBng="0" cellpadding="0" id="main-tab">
      <tr  class="bggray">
      ...
      <%
      		List<Kqysjyxjlb> list = (List)request.getSession().getAttribute("kqysjyxjlblist");
      		if(list != null) {
      ...
      %>
			      ...
			      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
			        ...
			        <th align="center" valign="middle"><%=list.get(i).getPaiqiyali() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getXitongyali() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getLengqueshuiyali() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getRunhuayouyali() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getPaiqiwendu1() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getPaiqiwendu2() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getXitongwendu() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getHuanjingwendu() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getRunhuayouwendu() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getLengqueshuiwendu() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getQzhudianjiwendu() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getHzhudianjiwendu() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getYunzhuandianya() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getDianliuzhishi() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getLeijiyunxingshijian() %></th>
			        <th align="center" valign="middle"><%=list.get(i).getLeijifuheshijian() %></th>
			        ...
			        <th align="center" valign="middle"><%=list.get(i).getMark() %></th>
				        <td align="center" valign="middle" class="borderbottom">
				         ...
				         %>
				        	<a href="Kqysjyxjlb.html?no=<%=request.getSession().getAttribute("name").toString() %>&id=<%=list.get(i).getId() %>" disabled target="mainFrame" onFocus="this.blur()" class="updateData" style="display:<%=classNone%>">编辑</a>
				        	<span class="gray" style="display:<%=classNone%>">&nbsp;|&nbsp;</span>
					         ...
					         %>
				        	<a href="<%=request.getContextPath() %>/DeleteKQYSJYXJLB?id=<%=list.get(i).getId() %>" style="display:<%=classNone%>" onclick="return delDate(<%=list.get(i).getId() %>, this)" target="mainFrame" onFocus="this.blur()" class="DeleteeteData">删除</a>
				        	<!--
				            	作者：923501316@qq.com
				            	时间：2018-04-18
				            	描述：如果该记录被审核，则允许再次审核
				            ...
				            %>
				        	<span class="gray" style="display:<%=classNone %>">&nbsp;|&nbsp;</span>
				        	<a href="<%=request.getContextPath() %>/SHKQYSJYXJLB?id=<%=list.get(i).getId() %>&status=1" target="mainFrame" onFocus="this.blur()" class="stateData" style="display:<%=classNone %>">通过</a>
				        	<span class="gray" style="display:<%=classNone %>">&nbsp;|&nbsp;</span>
				        	<a href="<%=request.getContextPath() %>/SHKQYSJYXJLB?id=<%=list.get(i).getId() %>&status=2" target="mainFrame" onFocus="this.blur()" class="stateData" style="display:<%=classNone %>">不通过</a>
				        </td>
		        <%
			        	}
		        	}
    ...
    %>
		  <tr>
		    <td align="left" valign="top" class="fenye">
	    		...
	    		<a href="<%=request.getContextPath() %>/newCareDevice/GetKQYSJYXJLB?page=1" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
		    	<%
		    		int beforePage = thisPage - 1;
		    		if(thisPage>1) {
		    		
		    	%>
		    		<a href="<%=request.getContextPath() %>/newCareDevice/GetKQYSJYXJLB?page=<%=beforePage %>" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;
		    	<% 
		    		}
		    	%>
		    	<%
		    	...
		    	%>
		    		<a href="<%=request.getContextPath() %>/newCareDevice/GetKQYSJYXJLB?page=<%=afterPage %>" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;
		    	<%
		    		}
		    	%>
		    	...
		    	%>
		    		<a href="<%=request.getContextPath() %>/newCareDevice/GetKQYSJYXJLB?page=<%=pageSize %>" target="mainFrame" onFocus="this.blur()">尾页</a></td>
		    	<%
		    		}
		    	%>
		  </tr>
	...
	<script>
	    ...
	    $("#export").on("click",function() {
      	  ...
      	  console.log(params)
	    		$.ajax({
					url: 'newCareDevice/GetKQYSJYXJLBPage',
					type: 'get',
					data:params,
					success: function(res) {
