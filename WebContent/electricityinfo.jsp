...
<%
	...
	if(session.getAttribute("pageSizeECI") != null )
		pageSize = Integer.parseInt(session.getAttribute("pageSizeECI").toString());
	if(session.getAttribute("thisPageECI")!=null)
		thisPage = Integer.parseInt(session.getAttribute("thisPageECI").toString());
	if(pageSize%15==0)
...
<script>
	var addActionTarget = "<%=request.getContextPath() %>/careDevice/InsertECI";
	var updateActionTarget = "<%=request.getContextPath() %>/careDevice/UpdateECI";
	var stateActionTarget = "<%=request.getContextPath() %>/careDevice/StateECI";
	var deleteActionTarget = "<%=request.getContextPath() %>/careDevice/DelECI";
</script>
</head>
<body>
<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
  <tr>
    <td width="99%" align="left" valign="top">您的位置：配电室看护信息</td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
  		...
  		%>
   		 <td width="88%" align="left" valign="middle">
	         <form method="post" action="<%=request.getContextPath() %>/careDevice/GetECI?page=1">
	         <span>设备单位：</span>
         ...
         %>
  		  <td width="10%" align="center" valign="middle" style="text-align:right; width:180px;display:<%=classNone%>"><a href="javascript:void(0);" target="mainFrame" onFocus="this.blur()" class="add" id="addDom">配电室看护信息</a></td>
  		</tr>
	</table>
    </td>
  </tr>
  <tr>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
      <tr  class="bggray">
        ...
        <th align="center" valign="middle" >高压配电</th>
        <th align="center" valign="middle" >低压配电</th>
      ...
      <%
      		List<ElectricityInfo> list = (List)request.getSession().getAttribute("listECI");
      		if(list != null) {
      ...
      %>
			      ...
			      <tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
			        ...
			        <th align="center" valign="middle" ><%=list.get(i).getBottomPiezoelectricity() %></th>
			        ...
			        %>
				        <td align="center" valign="middle" class="borderbottom">
				        	...
				        	<a href="<%=request.getContextPath() %>/careDevice/DelECI?id=<%=list.get(i).getId() %>" style="display:<%=classNone%>;" target="mainFrame" onFocus="this.blur()" onclick="return deleteData()" class="deleteData">删除</a>
				        	<!--
				            	作者：923501316@qq.com
				            	时间：2018-04-18
				            	描述：如果该记录被审核，则允许再次审核
    ...
    %>
		  <tr>
		    <td align="left" valign="top" class="fenye">
	    		...
	    		<a href="<%=request.getContextPath() %>/careDevice/GetECI?page=1" target="mainFrame" onFocus="this.blur()">首页</a>&nbsp;&nbsp;
		    	<%
		    		int beforePage = thisPage - 1;
		    		if(thisPage>1) {
		    		
		    	%>
		    		<a href="<%=request.getContextPath() %>/careDevice/GetECI?page=<%=beforePage %>" target="mainFrame" onFocus="this.blur()">上一页</a>&nbsp;&nbsp;
		    	<% 
		    		}
		    	%>
		    	<%
		    	...
		    	%>
		    		<a href="<%=request.getContextPath() %>/careDevice/GetECI?page=<%=afterPage %>" target="mainFrame" onFocus="this.blur()">下一页</a>&nbsp;&nbsp;
		    	<%
		    		}
		    	%>
		    	...
		    	%>
		    		<a href="<%=request.getContextPath() %>/careDevice/GetECI?page=<%=pageSize %>" target="mainFrame" onFocus="this.blur()">尾页</a></td>
		    	<%
		    		}
		    	%>
		  </tr>
...
<div id="dataBox">
	...
	<div style="text-align: center;width:45%;">
		<div class="data_head">
			<h2><span id="dataBoxTitle"></span>配电室看护信息</h2>
		    	...
		    	<td align="left" valign="top">
			    	<form method="post" action="">
				    	<table width="90%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
							...
							<tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
								<td align="right" valign="middle" class=" bggray">高压配电：</td>
								<td align="left" valign="middle">
							...
							<tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
								<td align="right" valign="middle" class=" bggray">低压配电：</td>
								<td align="left" valign="middle">
									<input type="text" name="bottomPiezoelectricity" value="" stateDisabled >
								</td>
							</tr>
