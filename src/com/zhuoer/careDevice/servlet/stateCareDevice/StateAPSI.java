...
@WebServlet("/careDevice/StateAPSI")
public class StateAPSI extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(code == 0) {
			UpdataState.stateN("airpressurestationinfo", id, auditOpinion2, request.getSession().getAttribute("name").toString());
		}
		else if(code == 1) {
			UpdataState.stateY("airpressurestationinfo", id, auditOpinion2, request.getSession().getAttribute("name").toString());
		}
