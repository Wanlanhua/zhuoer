...
/**
 * Servlet implementation class InsertPDSDY
 */
@WebServlet("/InsertPDSDY")
public class InsertPDSDY extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean flag = InsertDao.insertPdsdy(qt);
		if(flag) {
			OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), "看护管理=>配电室低压表=>添加操作");
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(flag) {
			result="1";
			OpLogInfoTools.insertOpLogInfo(qt.getJiluren(), "看护管理=>配电室低压表=>添加操作");
		}
