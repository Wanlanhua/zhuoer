...
/**
 * Servlet implementation class DeleteQZXJJLB
 */
@WebServlet("/DeleteQZXJJLB")
public class DeleteQZXJJLB extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DeleteQZXJJLB() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean flag = DeleteDao.deleteQzxjjlb(Integer.valueOf(id));
		JsonData<List> jd = new JsonData<>();
		if(flag) {
			OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), "看护管理=>气站巡检记录表=>删除操作");
			jd.setStatus(true);
			jd.setMessage("删除成功");
		} else {
			jd.setStatus(false);
