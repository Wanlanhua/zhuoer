...
/**
 * Servlet implementation class DeleteDBCZJLB
 */
@WebServlet("/DeleteDBCZJLB")
public class DeleteDBCZJLB extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DeleteDBCZJLB() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean flag = DeleteDao.deleteDbczjlb(Integer.valueOf(id));
		JsonData<List> jd = new JsonData<>();
		if(flag) {
			OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), "看护管理=>电表充值记录表=>删除操作");
			jd.setStatus(true);
			jd.setMessage("删除成功");
		} else {
			jd.setStatus(false);
