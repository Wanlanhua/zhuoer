...
/**
 * Servlet implementation class DeletePDSDY
 */
@WebServlet("/DeletePDSDY")
public class DeletePDSDY extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean flag = DeleteDao.deletepdsdy(Integer.valueOf(id));
		JsonData<List> jd = new JsonData<>();
		if(flag) {
			OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), "看护管理=>配电室低压=>删除操作");
			jd.setStatus(true);
			jd.setMessage("删除成功");
		} else {
			jd.setStatus(false);
