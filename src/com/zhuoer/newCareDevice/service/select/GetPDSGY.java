...
/**
 * Servlet implementation class GetPDSGY
 */
@WebServlet("/newCareDevice/GetPDSGY")
public class GetPDSGY extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getSession().getAttribute("thispdsgyPage")!=null && request.getSession().getAttribute("thispdsgyPage")!="") {
			page = request.getSession().getAttribute("thispdsgyPage").toString();
		}
		...
		List<Pdsgy> list = SelectDao.getPdsgy(departmentid, shebei, starttime, endtime, Integer.valueOf(page));
		int count = GetPageSize.getPageSize(shebei, "pdsgy", departmentid, starttime, endtime);
		...
		request.getSession().setAttribute("pdsgylist", list);
		request.getSession().setAttribute("thispdsgyPage", page);
		response.sendRedirect(request.getContextPath()+"/pdsgy.jsp");
	}
