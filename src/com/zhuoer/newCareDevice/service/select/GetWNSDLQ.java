...
/**
 * Servlet implementation class GetWNSDLQ
 */
@WebServlet("/newCareDevice/GetWNSDLQ")
public class GetWNSDLQ extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getSession().getAttribute("thiswnsdlqPage")!=null) {
			page = request.getSession().getAttribute("thiswnsdlqPage").toString();
		}
		...
		List<Wnsdlq> list = SelectDao.getWnsdlq(departmentid, shebei, starttime, endtime, Integer.valueOf(page));
		int count = GetPageSize.getPageSize(shebei, "wnsdlq", departmentid, starttime, endtime);
		...
		request.getSession().setAttribute("wnsdlqlist", list);
		request.getSession().setAttribute("thiswnsdlqPage", page);
		response.sendRedirect(request.getContextPath()+"/wnsdlq.jsp");
	}
