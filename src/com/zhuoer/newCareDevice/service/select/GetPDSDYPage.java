...
/**
 * Servlet implementation class GetPDSDYPage
 */
@WebServlet("/newCareDevice/GetPDSDYPage")
public class GetPDSDYPage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Pdsdy> list = SelectDao.getPdsdy(departmentid, shebei, starttime, endtime);
		if(list.size()>0) {
			long subffix = new Date().getTime();
			File xlsFile = new File(request.getServletContext().getRealPath("/")+"file/配电室低压"+subffix+".xls");
			String[] arrs = {"时间","设备","金额","有功总","有功需量","有功尖值",
					"有功峰值","有功平值","有功谷值","无功1","无功2","无功3",
					"无功4","功率因数","实际功率因数","倍率","电压","电流","记录人","备注"};
			Label label = null;
			try {
				...
				for(int j=0;j<list.size(); j++) {
					...
					sheet.addCell(label);
					label = new Label(15,j+1,list.get(j).getBeilv());
					sheet.addCell(label);
					label = new Label(16,j+1,list.get(j).getDianya());
					sheet.addCell(label);
					label = new Label(17,j+1,String.valueOf(list.get(j).getDianliu()));
					sheet.addCell(label);
					label = new Label(18,j+1,String.valueOf(list.get(j).getJiluren()));
					sheet.addCell(label);
					label = new Label(19,j+1,String.valueOf(list.get(j).getMark()));
					sheet.addCell(label);
			...
			response.getWriter().println("file/配电室低压"+subffix+".xls");
		}
		else {
