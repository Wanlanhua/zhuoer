...
/**
 * Servlet implementation class GetJZGQZLLJLB
 */
@WebServlet("/newCareDevice/GetJZGQZLLJLB")
public class GetJZGQZLLJLB extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GetJZGQZLLJLB() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getSession().getAttribute("thisjzgqzlljlbPage")!=null) {
			page = request.getSession().getAttribute("thisjzgqzlljlbPage").toString();
		}
		...
		List<Jzgqzlljlb> list = SelectDao.getJzgqzlljlb(departmentid, shebei, starttime, endtime, Integer.valueOf(page));
		int count = GetPageSize.getPageSize(shebei, "jzgqzlljlb", departmentid, starttime, endtime);
		...
		request.getSession().setAttribute("jzgqzlljlblist", list);
		request.getSession().setAttribute("thisjzgqzlljlbPage", page);
		response.sendRedirect(request.getContextPath()+"/jzgqzlljlb.jsp");
	}
