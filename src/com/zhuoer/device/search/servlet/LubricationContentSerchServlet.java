 ...
 */
@WebServlet("/LubricationContentSerchServlet")
public class LubricationContentSerchServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public LubricationContentSerchServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("admin".equals(no)) {
			String s = "select content from lubricationInfo where content like ?";
			List<String> mainNameList = MaintenanceDao.selectContentToList(s, "%"+content+"%");
			out.print(gson.toJson(mainNameList));
		}else {
			String s = "select content from lubricationInfo where customerNo in (select department from employeeinfo where no = ?) and content like ?";
