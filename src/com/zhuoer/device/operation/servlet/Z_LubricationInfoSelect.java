 ...
 */
@WebServlet("/Z_LubricationInfoSelect")
public class Z_LubricationInfoSelect extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Z_LubricationInfoSelect() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		sql = "select * from lubricationInfo where customerNo = ? and content like ? limit ?,15 ";
		sql2="SELECT COUNT(*) FROM lubricationInfo where customerNo = ? and content like ?";
		p.add(customerNo);
		p.add("%"+content+"%");
		p.add((page-1)*15);
		p2.add(customerNo);
