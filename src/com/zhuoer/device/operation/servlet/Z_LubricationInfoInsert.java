 ...
 */
@WebServlet("/Z_LubricationInfoInsert")
public class Z_LubricationInfoInsert extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Z_LubricationInfoInsert() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sqlFind="select * from lubricationInfo where content='"+content+"'";
		List<Map<String, Object>> listAll=RepairInfoTools.executeQuary(sqlFind);
		if (listAll.size()>0) {
			map.put("message", "该润滑项名称已存在!");
			String nameString = gson.toJson(map);
			out.println(nameString);
			return;
		}
		String sql = "insert into lubricationInfo(customerNo,content,stamp,mark)values(?,?,?,?)";
