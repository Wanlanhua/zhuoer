 ...
 */
@WebServlet("/Z_DandMInfoInsertServlet")
public class Z_DandMInfoInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Z_DandMInfoInsertServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		p.add(stamp);
		p.add(mark);
		String sqlFind="select * from DandMInfo where deviceNo='"+deviceNo+"' and name='"+name+"'";
		List<Map<String, Object>> listAll=RepairInfoTools.executeQuary(sqlFind);
		if (listAll.size()>0) {
			Map<String, String> map = new HashMap();
			map.put("message", "该保养项已关联!");
			String nameString = gson.toJson(map);
			out.println(nameString);
			return;
		}
		String sql = "insert into DandMInfo(deviceNo,name,stamp,mark)values(?,?,?,?)";
