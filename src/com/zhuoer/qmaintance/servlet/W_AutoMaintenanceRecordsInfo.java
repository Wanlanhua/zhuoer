 ...
 */
@WebServlet("/W_AutoMaintenanceRecordsInfo")
public class W_AutoMaintenanceRecordsInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//分用户获取需要进行推送的报修信息
		if (no.equals("admin")) {
			sqlAll="SELECT * FROM MaintenanceRecordsInfo  WHERE state2!='1' ";
			if(startdate!=null && !startdate.equals(""))
			{
				sqlAll+=" and maintenanceDate>='"+startdate+"'";
			}
			if(enddate!=null && !enddate.equals(""))
			{
				sqlAll+=" and maintenanceDate<='"+enddate+"'";
			}
			
		}else
		{
			
			sqlAll="SELECT m.* FROM MaintenanceRecordsInfo m,deviceinfo d,employeeinfo e WHERE m.state2!='1' AND m.deviceNo=d.no  AND d.customerNo=e.department AND e.no='"+no+"'";
			if(startdate!=null && !startdate.equals(""))
			{
				sqlAll+=" and maintenanceDate>='"+startdate+"'";
			}
