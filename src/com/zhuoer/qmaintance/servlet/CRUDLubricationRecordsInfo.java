 ...
 */
@WebServlet("/CRUDLubricationRecordsInfo")
public class CRUDLubricationRecordsInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "add" ://添加数据
			OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), 
					"保养维修=>润滑管理=>增加操作");
			int i=MaintenanceRecordsTools.addLubricationRecordsInfo(mr);
			if(i>0)
			{
				Map m=new HashMap();
				m.put("message", "ok");
		...
		case "del"://删除数据
			OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), 
					"保养维修=>润滑管理=>删除操作：" + mr.getId());
			String sql="delete from LubricationRecordsInfo where id=?";
		...
		case "alt"://修改数据
			OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), 
					"保养维修=>润滑管理=>修改操作：" + mr.getId());
			String state1=request.getParameter("state1");
			String state2=request.getParameter("state2");
			String sql2="update LubricationRecordsInfo set deviceNo=?,content=?,part=?,"
		...
		case "one"://一级审核
			if (mr.getState().equals("1"))
				OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), 
						"保养维修=>润滑管理=>一级审核通过：" + mr.getId());
			else
				OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), 
						"保养维修=>润滑管理=>一级审核未通过：" + mr.getId());
			String sql3="update LubricationRecordsInfo set state=?,auditOpinion1=? where id=?";
		...
		case "two"://二级审核
			if (mr.getState().equals("3"))
				OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), 
						"保养维修=>润滑管理=>二级审核通过：" + mr.getId());
			else
				OpLogInfoTools.insertOpLogInfo(request.getSession().getAttribute("name").toString(), 
						"保养维修=>润滑管理=>二级审核未通过：" + mr.getId());
			String sql4="update LubricationRecordsInfo set state=?,auditOpinion2=? where id=?";
