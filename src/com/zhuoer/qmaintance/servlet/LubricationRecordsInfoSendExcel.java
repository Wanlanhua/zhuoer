 ...
 */
@WebServlet("/LubricationRecordsInfoSendExcel")
public class LubricationRecordsInfoSendExcel extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sql="select lubricationRecordsInfo.*,deviceinfo.name as deviceinfoname from deviceinfo,lubricationRecordsInfo where  deviceinfo.no=lubricationRecordsInfo.deviceNo";
		...
		if(records.size()>0) {
			...
			File xlsFile = new File(request.getServletContext().getRealPath("/")+"file/润滑记录表"+subffix+".xls");
			String[] arrs = {"审核状态","一级审核意见","二级审核意见","润滑项目",
					"标准与工时","指派人","润滑周期","润滑日期","领取状态","完成状态",
			...
			try {
				...
				for(int j=0;j<records.size(); j++) {
					...
					sheet.addCell(label);
//					label = new Label(7,j+1, records.get(j).get("security")==null?"无":(records.get(j).get("security").toString()));
//					sheet.addCell(label);
//					label = new Label(8,j+1, records.get(j).get("tool")==null?"无":(records.get(j).get("tool").toString()));
//					sheet.addCell(label);
			...
			response.getWriter().println("file/润滑记录表"+subffix+".xls");
		}
		else {
