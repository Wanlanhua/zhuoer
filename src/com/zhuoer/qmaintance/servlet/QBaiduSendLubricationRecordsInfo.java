...
/**安卓润滑推送
 * Servlet implementation class QBaiduSendLubricationRecordsInfo
 */
@WebServlet("/QBaiduSendLubricationRecordsInfo")
public class QBaiduSendLubricationRecordsInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=0;i<split.length;i++)
		{
			...
			//String sql2="select * from lubricationRecordsInfo where deviceNo=? and state2 in(0,2)";//未领取，
			String sql2="select * from lubricationRecordsInfo where id=? ";//所有的都可以推送，
			...
			//推送给指定人
				String message="请润滑认领";
				JSONObject json=new JSONObject();
		    	json.put("title", "润滑指定认领");
		    	json.put("description", message+"~"+id+"~"+deviceNo+"~"+state1+"~"+"2"+"~"+state2);//内容+id+deviceNo+state1+type+state2
