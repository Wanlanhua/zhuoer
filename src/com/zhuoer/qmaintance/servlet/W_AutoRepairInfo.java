 ...
 */
@WebServlet("/W_AutoRepairInfo")
public class W_AutoRepairInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (no.equals("admin")) {
			...
			{
				sqlAll+=" and repairDate>='"+startdate+"'";
			}
			if(enddate!=null && !enddate.equals(""))
			{
				sqlAll+=" and repairDate<='"+enddate+"'";
			}
			
		}else
		{
			sqlAll="SELECT l.* FROM repairinfo l,deviceinfo d,employeeinfo e WHERE l.state2!='1' AND l.deviceNo=d.no  AND d.customerNo=e.department AND e.no='"+no+"'";
			if(startdate!=null && !startdate.equals(""))
			{
				sqlAll+=" and repairDate>='"+startdate+"'";
			}
