...
/**自动推送定保信息接口
 * Servlet implementation class AutoMaintenanceRecordsInfo
 */
@WebServlet("/AutoMaintenanceRecordsInfo")
public class AutoMaintenanceRecordsInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//分用户获取需要进行推送的报修信息
		if (no.equals("admin")) {
			sqlAll="SELECT * FROM MaintenanceRecordsInfo  WHERE state2!='1' and DATEDIFF(CURDATE(),maintenanceDate) BETWEEN -2 and 2";
			
		}else
		{
			sqlAll="SELECT m.* FROM MaintenanceRecordsInfo m,deviceinfo d,employeeinfo e WHERE m.state2!='1' and DATEDIFF(CURDATE(),maintenanceDate) BETWEEN -2 and 2 AND m.deviceNo=d.no  AND d.customerNo=e.department AND e.no='"+no+"'";
		}
...
//					else//没有指派人条件下
//					{
//						String msg="定保认领~"+listAll.get(i).get("id").toString()+"~"+listAll.get(i).get("deviceNo").toString()+"~"+listAll.get(i).get("state1").toString()+"~"+"0"+"~"+listAll.get(i).get("state2").toString();
//						String channelId=listM.get(j).get("channelId").toString();
//						BaiDuTools.sendBaidu(channelId, msg);
//						sendMsg="推送成功！";
//					}
